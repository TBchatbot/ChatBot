...
# Custom CSS for styling chat messages and UI elements
st.markdown("""
<style>
    .chat-message {
        padding: 1rem;
...
# Function to find relevant responses based on input
...
# Initialize session state for chat messages and state
...
# Render the welcome screen before chat starts
...
# Render sidebar with quick topic buttons and emergency info
def render_sidebar():
    with st.sidebar:
...
# Render individual chat messages with avatar and timestamp
...
# Render the main chat interface with header, messages and input box
